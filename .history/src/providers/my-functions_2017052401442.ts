...
export class MyFunctions {
  ...
  getMachineStatus(){
    return new Promise(resolve => {
      this.myDatabase.dbQueryRes("SELECT machines.owner_id AS owner_id, clients.last_order_date AS client_last_order_date, (SELECT orders.created_date FROM orders WHERE orders.client_id = clients.id ORDER BY orders.created_date DESC LIMIT 1) AS last_order FROM machines LEFT OUTER JOIN clients ON machines.owner_id = clients.id WHERE machines.creator_id = ?", [this.myDatabase.userInfo.id]).then((data:any)=>{
        var ar
        var arr = []
        for(var x in data){
          ar = data[x]
