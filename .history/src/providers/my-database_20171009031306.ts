...
import {SQLite, Device, AppVersion, File, SocialSharing } from 'ionic-native';
...
export class MyDatabase {
  ...
  constructor(public http: Http, public alertCtrl: AlertController, public loadingCtrl: LoadingController, public sanitizer: DomSanitizer, public actionSheetCtrl: ActionSheetController, private nativeStorage: NativeStorage) {
    ...
    if(!this.isOpen){
      ...
      this.getSettings('lastSync').then((data:any) => {
        if(data == "No Data" || "undefined" === typeof data){
          this.setSettings("lastSync", 'N/A')
          this.lastSync = 'N/A'
        }
        else{
          this.lastSync = data
      ...
      this.getSettings("depot").then((d) => {
        ...
        if(d == "No Data" || "undefined" === typeof d){
  ...
  getSettings(name){
    return new Promise((resolve) => {
      ...
      .then(
        data => {console.log(data);resolve(data)},
        error => resolve("No Data")
      );
    })
  }
  ...
  public loadDatabase(){
    return new Promise((resolve) => {
      ...
      this.storage.openDatabase({name: "data.db", location: "default"}).then(() => {
        ...
        //table creation
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, name TEXT NOT NULL DEFAULT '', created_date INT, updated_date INT, photo TEXT NOT NULL DEFAULT '', contact TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS clients (id INTEGER PRIMARY KEY, name TEXT NOT NULL DEFAULT '', alias TEXT NOT NULL DEFAULT '', company TEXT NOT NULL DEFAULT '', contact TEXT NOT NULL DEFAULT '', contact2 TEXT NOT NULL DEFAULT '', photo TEXT NOT NULL DEFAULT '', lat REAL, lng REAL, last_order_date INT, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', email TEXT, thumbnail TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS machines (id INTEGER PRIMARY KEY NOT NULL, creator_id TEXT, owner_id TEXT, qr_id TEXT, created_date INT NOT NULL, updated_date INT NOT NULL, last_visit_date INT, maintenance_date INT, region TEXT NOT NULL DEFAULT '', municipal TEXT NOT NULL DEFAULT '', brgy TEXT NOT NULL DEFAULT '', accuracy REAL NOT NULL DEFAULT 0, delivery TEXT NOT NULL DEFAULT 'Unspecified', lat REAL NOT NULL DEFAULT 0, lng REAL NOT NULL DEFAULT 0, machine_type TEXT NOT NULL DEFAULT '', sequence INT NOT NULL DEFAULT 0, photo TEXT NOT NULL DEFAULT '', thumbnail TEXT, establishment_type TEXT, report_date INT, sync INT)", []);
        ...
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS disr (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', sequence_id TEXT, parent_id INTEGER, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS disr_payments (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', disr_id INTEGER, amount REAL, notes TEXT, status TEXT, alarm INTEGER, photo TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', status TEXT, client_id INTEGER, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS order_payments (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', amount REAL, order_id INTEGER, change REAL, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS disr_items (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', plus INTEGER NOT NULL DEFAULT 0, minus INTEGER NOT NULL DEFAULT 0, type TEXT, order_id INTEGER, disr_id INTEGER, product_id INTEGER, remaining INT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS actions (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT NOT NULL DEFAULT '', name TEXT, action TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS callsheets (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT, machine_id INT, order_id INT, type TEXT, lat REAL, lng REAL, note TEXT, distance REAL, sync INT)", []);
        ...
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS admin_contacts (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT, name TEXT, contact TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS reporting_contacts (id INTEGER PRIMARY KEY, created_date INT, updated_date INT, creator_id TEXT, number TEXT, type TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS sms_unsent (id INTEGER PRIMARY KEY, creator_id TEXT, msg TEXT, numbers TEXT, sync INT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS carriers (id INTEGER PRIMARY KEY, carrier TEXT, prefix TEXT)", []);
        this.storage.executeSql("CREATE TABLE IF NOT EXISTS coverage_area (id INTEGER PRIMARY KEY, creator_id TEXT, area TEXT, sync INT)", []);
        ...
        this.dbQuery("SELECT COUNT(id) AS mycount FROM products ", []).then((data:any) => {
          if(!data.rows.item(0).mycount){
            ...
            let myArr = [
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [1, this.getTimestamp(), this.getTimestamp(), "Milky Choco Loco", "Powder Mix", 190, "kg", "", "pm_mcl", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [2, this.getTimestamp(), this.getTimestamp(), "Caramel Macchiato", "Powder Mix", 190, "kg", "", "pm_cm", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [3, this.getTimestamp(), this.getTimestamp(), "Coffee Pinoy Blend", "Powder Mix", 190, "kg", "", "pm_cpb", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [4, this.getTimestamp(), this.getTimestamp(), "Coffee White", "Powder Mix", 190, "kg", "", "pm_cw", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [5, this.getTimestamp(), this.getTimestamp(), "Coffee Brown", "Powder Mix", 190, "kg", "", "pm_cb", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [6, this.getTimestamp(), this.getTimestamp(), "Creamy Milk Tea", "Powder Mix", 190, "kg", "", "pm_cmt", 0, ""]],
              ["INSERT OR REPLACE INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [7, this.getTimestamp(), this.getTimestamp(), "Mocha Cappuccino", "Powder Mix", 190, "kg", "", "pm_mc", 0, ""]],
        ...
        this.addColumn("users", "sync", "INT")
        this.addColumn("disr", "sync", "INT")
        this.addColumn("disr_payments", "sync", "INT")
        this.addColumn("orders", "sync", "INT")
        this.addColumn("order_payments", "sync", "INT")
        this.addColumn("disr_items", "sync", "INT")
        this.addColumn("actions", "sync", "INT")
        this.addColumn("callsheets", "sync", "INT")
        this.addColumn("admin_contacts", "sync", "INT")

        this.addColumn("reporting_contacts", "sync", "INT")
        this.addColumn("sms_unsent", "sync", "INT")
        this.addColumn("coverage_area", "sync", "INT")
  ...
  fullBackUp(){
    ...
    function writeNow(path, fName, sql){
      ...
      }).catch(e => {console.log(e); self.hideLoading()})
    }
  ...
  restoreDb(uri){
    let self = this
    self.setSettings("userInfo", "")
    this.userInfo = false
    var progressFn = function(current, total){
      console.log("Imported "+current+"/"+total+" statements");
      self.loading.setContent("Imported "+current+"/"+total+" statements")
  ...
  setQty(disr_id, product_id, value, type, set, order_id = null){
    return new Promise((resolve, reject) => {
      this.dbQueryRes("SELECT * FROM disr_items WHERE creator_id = ? AND disr_id = ? AND product_id = ?", [this.userInfo.id, disr_id, product_id]).then((res:any) => {
        ...
        this.dbQueryRes("INSERT OR REPLACE INTO disr_items VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [t, t, t, this.userInfo.id, parseInt(add), parseInt(minus), type, order_id, disr_id, product_id, null, null]).then((done:any) => {
          resolve(true)
        })
      })
    })
