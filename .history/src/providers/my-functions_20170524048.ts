...
export class MyFunctions {
  ...
  getMachineStatus(){
    return new Promise(resolve => {
      this.myDatabase.dbQueryRes("SELECT machines.owner_id AS owner_id, clients.last_order_date AS client_last_order_date, (SELECT t2.created_date FROM orders t2 WHERE t2.client_id = t3.id ORDER BY t2.created_date DESC LIMIT 1) FROM machines t1 LEFT OUTER JOIN clients t3 ON machines.owner_id = clients.id WHERE machines.creator_id = ?", [this.myDatabase.userInfo.id]).then((data:any)=>{
        var ar
        var arr = []
        for(var x in data){
          ar = data[x]
          ar.client_status = (ar.client_last_order_date) ? "active" : (ar.owner_id) ? "prospect" : "lead"
          arr.push(ar)
        }
