...
export class MyFunctions {
  ...
  getMachineStatus(){
    return new Promise(resolve => {
      this.myDatabase.dbQueryRes("SELECT t1.owner_id AS owner_id, t3.last_order_date AS client_last_order_date, (SELECT t2.created_date FROM orders t2 WHERE t2.client_id = t3.id ORDER BY t2.created_date DESC LIMIT 1) AS last_order FROM machines t1 LEFT OUTER JOIN clients t3 ON t1.owner_id = t3.id WHERE machines.creator_id = ?", [this.myDatabase.userInfo.id]).then((data:any)=>{
        var ar
        var arr = []
        for(var x in data){
          ar = data[x]
