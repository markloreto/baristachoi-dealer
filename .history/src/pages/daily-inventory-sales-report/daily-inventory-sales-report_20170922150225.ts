...
export class DailyInventorySalesReportPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    this.myDatabase.dbQueryRes("SELECT COUNT(id) AS totalA FROM disr WHERE creator_id = ?", [this.myDatabase.userInfo.id]).then((t:any) => {
      ...
      console.log("DISR Count", this.disrCount)
    })
  }
        ...
        },
        {
          ...
          handler: data => {
            ...
            if(data.sequence != ""){
              ...
              this.myDatabase.dbQueryRes("INSERT INTO disr (id, created_date, updated_date, creator_id, status, sequence_id, parent_id, sync) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [t, t, t, this.myDatabase.userInfo.id, "active", data.sequence.replace(/[^A-Z0-9]+/ig, "").replace(/\s/gim, ""), null, null]).then((res:any) => {
