            ...
            function gotFile(fileEntry) {
              fileEntry.file(function(file) {
                ...
                reader.onloadend = function(e) {
                  ...
                  resolve({content: content, dir: filePath})
                };
  ...
  machineThumbnail(i){
    this.myDatabase.dbQueryRes("SELECT photo FROM machines WHERE id = ?", [this.machineIds[i].id]).then((data:any) => {
      ...
      if(data[0].photo){
        ...
        this.resizeIt(this.convertDirectory, this.machineIds[i].id + ".jpg", 100, 128, 96).then((content: any) => {
          ...
          this.myDatabase.dbQueryRes("UPDATE machines SET thumbnail = ? WHERE id = ?", [content.content, this.machineIds[i].id]).then(_ => {
            File.removeFile(content.dir).then((res:any) => {this.navCtrl.pop();}).catch(err => {this.navCtrl.pop();})
            i = i + 1
