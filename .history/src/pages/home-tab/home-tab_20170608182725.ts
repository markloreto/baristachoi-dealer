...
export class HomeTabPage {
  ...
  machineThumbnail(i){
    this.myDatabase.dbQueryRes("SELECT photo FROM machines WHERE id = ?", [this.machineThumbnail[i].id]).then((data:any) => {
      ...
      this.savebase64AsImageFile(this.machineThumbnail[i].id + ".jpg", data[0].photo.replace('data:image/jpeg;base64,', ''), 'image/jpeg', this.convertDirectory).then(() => {
        ...
        this.resizeIt(this.convertDirectory, this.machineThumbnail[i].id + ".jpg", 100, 128, 96).then((base64: any) => {
