...
export class HelloIonicPage {
  ...
  warningStock: any = []
  constructor(public myDatabase: MyDatabase, public myFunction: MyFunctions, public alertCtrl: AlertController, private barcodeScanner: BarcodeScanner, private fcm: FCM) {
      ...
      for(var x in a){
        if(a[x].beginning != 0 || a[x].previous != 0 || a[x].reload){
          ...
          if(c != 0 && c >= a[x].remaining)
            b.push({remaining: a[x].remaining, name: a[x].c_name, threshold: c})
        }
      }
