...
export class HelloIonicPage {
  ...
  constructor(public myDatabase: MyDatabase, public myFunction: MyFunctions, public alertCtrl: AlertController, private barcodeScanner: BarcodeScanner) {
    this.myFunction.getCurrentGPSLocation().then((location: any) => {
      this.myDatabase.dbQueryRes("SELECT * FROM machines ORDER BY ((lat-?)*(lat-?)) + ((lng - ?)*(lng - ?)) ASC", [location.latitude, location.latitude, location.longitude, location.longitude])
    })
