...
export class PointOfSalePage {
  ...
  pos: any
  distance:any
  constructor(public navCtrl: NavController, public navParams: NavParams, public myFunctions: MyFunctions, public myDatabase: MyDatabase, private alertCtrl: AlertController) {
  ...
  paymentStart(amount){
    if(amount >= this.total){
      ...
      for(var x in this.items){
        prd = this.items[x]
        q.push(["INSERT INTO disr_items VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [id+(x+1), id, id, this.myDatabase.userInfo.id, 0, prd.qty, "Sold", id, parseInt(this.disrId), prd.id, null]])
      }

      q.push(["INSERT INTO orders VALUES (?, ?, ?, ?, ?, ?)", [id, id, id, this.myDatabase.userInfo.id, "Paid", this.clientId]])
      q.push(["INSERT INTO order_payments VALUES(?, ?, ?, ?, ?, ?, ?)", [id, id, id, this.myDatabase.userInfo.id, amount, id, change]])
      q.push(["INSERT INTO callsheets VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [id, id, id, this.myDatabase.userInfo.id, this.machineId, id, "Sale", this.pos.coords.latitude, this.pos.coords.longitude, amount-change, this.distance]])
      if(this.distance <= this.myFunctions.gpsMinimumMeter || this.distance == null){
        q.push(["UPDATE machines SET last_visit_date = ?, updated_date = ? WHERE creator_id = ? AND id = ?", [id, id, this.myDatabase.userInfo.id, this.machineId]])
        q.push(["UPDATE clients SET last_order_date = ? WHERE creator_id = ? AND id = ?", [id, this.myDatabase.userInfo.id, this.clientId]])
      }
      ...
      console.log("Less that the total")
    }
  }
