...
export class PointOfSalePage {
    ...
    function s(){
      ...
      self.speech(speak).then(() => {
        ...
        if(i < self.items.length){
          ...
          var t = (self.total % 1 == 0) ? parseFloat(self.total + "") : self.total
  ...
  checkOutStart(){
    if(this.items.length != 0){
      var tot = (this.total % 1 == 0) ? parseFloat(this.total + "") : this.total
  ...
  paymentStart(amount){
    ...
    if(amount >= this.total){
      ...
      if(change > 0){
        ...
        setTimeout(() => {
          this.speech("I Received " + amount + " pesos, your change is " + change + " pesos. Have a good day " + this.selectedMachine.client_name)
        }, 1500)
      }else{
        setTimeout(() => {
          this.speech("I Received " + amount + " pesos. Have a good day " + this.selectedMachine.client_name)
        }, 1500)
      }
