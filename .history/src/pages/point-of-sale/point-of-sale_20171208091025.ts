...
export class PointOfSalePage {
    ...
    function s(){
      ...
      self.speech(speak).then(() => {
        ...
        if(i < self.items.length){
          ...
          var t = (self.total % 1 == 0) ? parseFloat((self.total + "").replace(/(\d)(?=(\d{3})+\.)/g, '$1,')) : self.total
          self.speech("With a total price of " + t + " pesos")
        }
      })
    }
    s()
  ...
  checkOutStart(){
    if(this.items.length != 0){
      var tot = (this.total % 1 == 0) ? parseFloat((this.total + "").replace(/(\d)(?=(\d{3})+\.)/g, '$1,')) : this.total
      this.speech("Please pay the amount of " + tot + " pesos")
  ...
  paymentStart(amount){
    ...
    if(amount >= this.total){
      ...
      if(change > 0){
        ...
        setTimeout(() => {
          this.speech("I Received " + parseFloat((amount + "").replace(/(\d)(?=(\d{3})+\.)/g, '$1,')) + " pesos, your change is " + parseFloat((change + "").replace(/(\d)(?=(\d{3})+\.)/g, '$1,')) + " pesos. Have a good day " + this.selectedMachine.client_name)
        }, 1500)
      }else{
        setTimeout(() => {
          this.speech("I Received " + parseFloat((amount + "").replace(/(\d)(?=(\d{3})+\.)/g, '$1,')) + " pesos. Have a good day " + this.selectedMachine.client_name)
        }, 1500)
      }
