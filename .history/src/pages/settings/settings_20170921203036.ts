...
export class SettingsPage {
  ...
  constructor(public navCtrl: NavController,
    ...
    this.depot = this.myDatabase.depot
    console.log("Depot:", this.depot)

    this.myFunctions.getDepot("depot").then((d: any) => {
      this.depotList = d.val
    })
  ...
  selectDepot(){
    var buttons = []
    for(var x in this.depotList){
      buttons.push({
        text: this.depotList[x].name,
        handler: this.myFunction.bind(this, x)
      })
    }
  ...
  myFunction(i){
    console.log(this.depotList[i])
    this.alertCtrl.create({
      title: 'Key Required',
      message: "Please input the Depot Key",
      inputs: [
        {
          name: 'key',
          placeholder: 'Key'
      ...
      buttons: [
        ...
        },
        {
          text: 'Unlock',
          handler: data => {
            if(data.key == this.depotList[i].password){
              this.myDatabase.setSettings("depot", this.depotList[i])
              this.myDatabase.depot = this.depotList[i]
              this.depot = this.depotList[i]
            }
            else{
              this.myFunctions.toastIt("Invalid Key")
            }
          }
        }
      ]
  ...
  capture(){
    Camera.getPicture({quality: 80, targetWidth: 1024, targetHeight: 768, destinationType: 0}).then((imageData) => {
      ...
      this.myDatabase.dbQueryRes("UPDATE users SET photo = ?, sync = ? WHERE id = ?", [base64Image, null, this.myDatabase.userInfo.id])
    ...
    this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          text: 'Save',
          handler: data => {
            this.myDatabase.dbQueryRes("UPDATE users SET name = ?, contact = ?, sync = ? WHERE id = ?", [data.name, data.contact, null, this.profile.id]).then(()=>{
  ...
  contactReportUpdate(id, numbers, type){
    ...
    this.myDatabase.dbQuery("INSERT OR REPLACE INTO reporting_contacts (id, creator_id, number, type, sync) VALUES (?, ?, ?, ?, ?)", [id, this.myDatabase.userInfo.id, numbers.join(), type, null])
  }
