...
export class CoverageAreaPage {
  ...
  set(area){
    this.myDatabase.showLoading()
    this.myDatabase.dbQueryRes("INSERT INTO coverage_area VALUES (?, ?, ?, ?)", [this.myDatabase.getTimestamp(), this.myDatabase.userInfo.id, area.region + ', ' + area.municipal + ', ' + area.brgy, null]).then(()=>{
      this.loadIt()
      this.myDatabase.hideLoading()
    })
  }
