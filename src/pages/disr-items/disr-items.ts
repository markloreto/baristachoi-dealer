...
export class DisrItemsPage {
  ...
  sendReport(){
    this.contacts.pickContact().then(c => {
      console.log(c)
      let name = c.name.formatted

      var k = []
      for(var x in c.phoneNumbers){
        k.push(this.myFunctions.getCarrier(c.phoneNumbers[x].value))
      }
      ...
      for(var x in c.phoneNumbers){
        ...
        btns.push({
          text: c.phoneNumbers[x].value + " ["+k[x]+"]",
          handler: () => {
            this.myFunctions.remainingStockReport(c.phoneNumbers[x].value)
          }
        })
      }
