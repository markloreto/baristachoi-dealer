...
export class AddMachinePage {
    ...
    this.myDatabase.dbQueryRes("SELECT * FROM coverage_area WHERE creator_id = ?", [this.myDatabase.userInfo.id]).then((ca:any) => {
      ...
      if(this.id){
        this.myDatabase.dbQuery("SELECT * FROM machines where id = ?",[this.id]).then((data:any) => {
          ...
          this.lat = mData.lat
          this.lng = mData.lng
          this.oldLat = mData.lat
          this.oldLng = mData.lng
  ...
  distance(lat, lng, lat2, lng2){
    let d = this.myFunctions.distance(lat, lng, lat2, lng2)
    return d
  }
  ...
  submitIt(ev){
    ...
    if(this.id){
      var v = [this.client.id, si, this.region, this.municipal, this.brgy, this.pos.coords.accuracy, this.delivery, this.lat, this.lng, this.machineType, this.photoRaw, this.establishmentType, this.thumb, null, this.id]
      this.myDatabase.dbQuery("UPDATE machines SET owner_id = ?, updated_date = ?, region = ?, municipal = ?, brgy = ?, accuracy = ?, delivery = ?, lat = ?, lng = ?, machine_type = ?, photo = ?, establishment_type = ?, thumbnail = ?, sync = ? WHERE id = ?", v).then((data:any) => {
        this.myFunctions.toastIt("Machine updated!");
        this.myDatabase.hideLoading()
        if(this.lat != this.oldLat || this.lng != this.oldLng){
          console.log("The location has changed. we need to report it")

          let d = parseFloat(this.distance(this.pos.coords.latitude, this.pos.coords.longitude, this.lat, this.lng))
          this.theDistance = parseFloat((d / 1000).toFixed(2))
          if(this.theDistance >= 1){
            this.myDatabase.dbQueryRes("SELECT * FROM reporting_contacts WHERE creator_id = ? AND type = ?", [this.myDatabase.userInfo.id, "general report"]).then((nc:any) => {
              if(nc.length){
                let myLat = this.myFunctions.myPos.coords.latitude
                let myLng = this.myFunctions.myPos.coords.longitude
                this.myFunctions.getLocation([myLat, myLng]).then((loc:any) => {

                  this.myFunctions.getLocation([this.oldLat, this.oldLng]).then((fromLoc:any) => {
                    let fromAddress = (fromLoc.NAME_1) ? fromLoc.NAME_1 + ", " + fromLoc.NAME_2 + ", " + fromLoc.NAME_3 : "Unknown"
                    this.myFunctions.getLocation([this.lat, this.lng]).then((toLoc:any) => {
                      let toAddress = (toLoc.NAME_1) ? toLoc.NAME_1 + ", " + toLoc.NAME_2 + ", " + toLoc.NAME_3 : "Unknown"

                      let myLoc = (loc.NAME_1) ? loc.NAME_1 + ", " + loc.NAME_2 + ", " + loc.NAME_3 : "Unknown"
                      let contacts = nc[0].number.split(",")
                      
                      let thisDate = moment().format("ddd, MMM D")
                      var msg = "Client Location Changes ("+thisDate+")\n"
                      if(this.client.name)
                        msg += "Name: " + this.client.name + "\n"
                      if(this.client.contact)
                        msg += "Contact: " + this.client.contact + "\n"
                      msg += "From: " + fromAddress + "\n"
                      msg += "To: " + toAddress + "\n"
                      msg += "Distance: " + this.theDistance + " Km \n"
                      msg += "Reporting from: " + myLoc + "\n\n"
                      
            
                      msg += "Map: http://maps.google.com/?q="+myLat+","+myLng
                      console.log(msg)
        ...
        File.removeFile(cordova.file.externalApplicationStorageDirectory + "machine_photo", this.id + ".jpg").then((res:any) => {

        }).catch(err => {this.navCtrl.pop();})
      })
    }else{
      var v = [si, this.myDatabase.userInfo.id, this.client.id, si, si, this.region, this.municipal, this.brgy, this.delivery, this.lat, this.lng, this.machineType, this.photoRaw, this.pos.coords.accuracy, this.establishmentType, this.thumb, null]
      this.myDatabase.dbQuery("INSERT INTO machines (id, creator_id, owner_id, created_date, updated_date, region, municipal, brgy, delivery, lat, lng, machine_type, photo, accuracy, establishment_type, thumbnail, sync) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )", v).then((data:any) => {
