...
export class AdminContactsPage {
  ...
  addAdminContact(name, number){
    ...
    this.myDatabase.dbQueryRes("INSERT INTO admin_contacts (id, created_date, updated_date, creator_id, name, contact, sync) VALUES (?, ?, ?, ?, ?, ?, ?)", [this.myDatabase.getTimestamp(), this.myDatabase.getTimestamp(), this.myDatabase.getTimestamp(), this.myDatabase.userInfo.id, name, number, null]).then(()=>{
      this.startIt()
    })
  }
